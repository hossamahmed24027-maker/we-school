...
export default async function ExamListPage(props: {
    ...
    const [rows, totalRows, lessons] = await Promise.all([
        pool.execute<any[]>(
            `SELECT e.id, e.title as name, e.startTime, e.endTime, e.lessonId, c.name as class, CONCAT(t.name, ' ', t.surname) as teacher, e.startTime as date
            FROM Exam e
            ...
            WHERE e.title LIKE ? OR s.name LIKE ? OR c.name LIKE ? OR t.name LIKE ? OR t.surname LIKE ?
            LIMIT ? OFFSET ?`,
