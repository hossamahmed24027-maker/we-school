...
export default async function SubjectListPage(props: {
    ...
    const [rows, totalRows, teachers] = await Promise.all([
        pool.execute<any[]>(
            `SELECT s.id, s.name,
                GROUP_CONCAT(CONCAT(t.name, ' ', t.surname) ORDER BY t.name SEPARATOR ', ') AS teachers,
                GROUP_CONCAT(t.id SEPARATOR ', ') AS teacherIds
            FROM Subject s
            LEFT JOIN _SubjectToTeacher st ON st.A = s.id
        ...
        pool.execute<any[]>(
            `SELECT COUNT(*) as total FROM Subject WHERE name LIKE ?`,
            [searchLike]
        ),
        pool.execute<any[]>(
        ...
        <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0 shadow-sm">
            <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                    ...
                    <div className="flex items-center gap-4 self-end">
                        <FormModal table="subject" type="create" relatedData={{ teachers: teachersData }} />
                    </div>
                </div>
            </div>
            <Table columns={columns} renderRow={(item: any) => (
                <tr key={item.id} className="border-b border-gray-200 even:bg-slate-50 text-sm hover:bg-lamaPurpleLight transition-colors">
                    ...
                    <td>
                        <div className="flex items-center gap-2">
                            <FormModal table="subject" type="update" data={item} relatedData={{ teachers: teachersData }} />
                            <FormModal table="subject" type="delete" id={item.id} />
                        </div>
                    </td>
                </tr>
