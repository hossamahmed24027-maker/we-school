...
export default function TeacherForm({
    ...
    const {
        ...
        defaultValues: {
            ...
            subjects: data?.subjectIds ? data.subjectIds.split(', ') : [],
        }
    });
    ...
    const onSubmit = handleSubmit(async (values) => {
        try {
            ...
            if (res.success) {
                toast.success(`Teacher ${type === "create" ? "registered" : "updated"} successfully! 🎓`);
                setOpen?.(false);
                router.refresh();
            } else {
    ...
    return (
        <form className="flex flex-col gap-8" onSubmit={onSubmit}>
            <h1 className="text-2xl font-black text-blue-900 tracking-tight">
                {type === "create" ? "Register New Teacher" : "Update teacher profile"}
            </h1>
            {data && <input type="hidden" {...register("id")} />}
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Auth Credentials</span>
            ...
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Identity Details</span>
            <div className="flex justify-between flex-wrap gap-4">
                ...
                <div className="flex flex-col gap-2 w-full md:w-[30%]">
                    <label className="text-xs font-bold text-slate-500">Subjects</label>
                    <select multiple className="p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 text-sm min-h-[100px]" {...register("subjects")}>
                    ...
                    {errors.subjects?.message && <p className="text-[10px] text-red-500 font-bold mt-1 px-1">{errors.subjects.message.toString()}</p>}
                </div>
            </div>
            <button className="bg-blue-600 text-white p-4 rounded-2xl font-black shadow-xl shadow-blue-100 hover:scale-105 active:scale-95 transition-all mt-4">
                {type === "create" ? "Register Member" : "Save Changes"}
            </button>
        </form>
    );
}
    ...
    return (
        <div className="flex flex-col gap-2 w-full md:w-[30%]">
            ...
            <input
                type={type}
                {...register(name)}
                className="p-3 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:border-blue-500 text-sm placeholder:text-slate-300 transition-all"
                {...inputProps}
            />
            {error?.message && <p className="text-[10px] text-red-500 font-bold mt-1 px-1">{error.message.toString()}</p>}
        </div>
    )
}
